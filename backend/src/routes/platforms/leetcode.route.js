...
router.get(
  '/user/language-stats',
  validate(usernameValidationSchema),
  LeetCodeController.getLanguageStats,
);
router.get(
  '/user/calendar',
  validate(usernameAndYearQueryValidationSchema),
  LeetCodeController.getUserCalendar,
);
router.get(
  '/user/recent-submissions',
  validate(recentSubmissionsValidationSchema),
  LeetCodeController.getRecentAcSubmissions,
);
router.get(
  '/user/badges',
  validate(usernameValidationSchema),
  LeetCodeController.getUserBadges,
);
router.get(
  '/user/contest-ranking',
  validate(usernameValidationSchema),
  LeetCodeController.getContestRanking,
);
router.get(
  '/user/skill-stats',
  validate(usernameValidationSchema),
  LeetCodeController.getSkillStats,
);
router.get(
  '/user/question-progress',
  validate(usernameValidationSchema),
  LeetCodeController.getUserProfileQuestionProgressV2,
);
router.get(
  '/user/session-progress',
  validate(usernameValidationSchema),
  LeetCodeController.getUserSessionProgress,
);
router.get(
  '/user/favorite-list',
  validate(usernameValidationSchema),
  LeetCodeController.getCreatedPublicFavoriteList,
);
router.get(
  '/contest/rating-distribution',
  LeetCodeController.getContestRatingHistogram,
);
router.get(
  '/coding-challenge/medal',
  validate(yearAndMonthQueryValidationSchema),
  LeetCodeController.getCodingChallengeMedal,
);
...
router.get(
  '/rankings/global',
  validate(paginationQueryValidationSchema),
  LeetCodeController.getGlobalTopRankers,
);
