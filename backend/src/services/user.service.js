...
const getSearchQuery = (searchField, searchOrder, cursor) => {
  ...
  return {
    $or: [
      ...
      {
        [searchField]: cursorValue,
        _id: { [secondaryCompare]: cursorId },
      },
    ],
  };
};
...
const getUser = async (displayName, currentUser, viewerDeviceToken, viewerSignedDeviceToken) => {
  ...
  const profileLinks = await ProfileModel.findOne({
    userId,
  }).select('-_id -userId -createdAt -updatedAt');
  queriedUser.profileLinks = profileLinks || {};

  const userProfileViewsModels = await ProfileViewModel.find({
    vieweeId: userId,
  });
...
const getUsers = async (params) => {
  ...
  if (cursor) {
    ...
    query = {
      $and: [fieldQuery, { name: { $regex: searchQuery, $options: 'i' } }],
    };
  } else {
    query = {
...
const getUserHighlights = async () => {
  const [totalUsers, sampleUsers, totalResumes, totalGithubProfiles, totalLeetcodeProfiles, totalApiKeys, publicApiData] = await Promise.all([
    ...
    ScoreModel.countDocuments({
      platform: {
        $in: ['Generic Resume', 'Resume with JD'],
      },
    }),
...
const updateUserApiKey = async (user, apiKey) => {
  const existingApiKey = await ApiProjectModel.findOne({
    apiKey,
  });
...
export { getUser, getUsers, updateUserInfo, updateDisplayName, changePassword, toggleProfileVisibility, getUserHighlights, toggle2FA, updateUserApiKey };
