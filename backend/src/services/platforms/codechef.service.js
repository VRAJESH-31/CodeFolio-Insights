    ...
    const data = await page.evaluate(
      async (username, includeAchievements, includeContests) => {
        ...
        if (includeContests) {
          ...
          const allRatings =
            typeof Drupal !== 'undefined' && Drupal.settings?.date_versus_rating?.all
              ? // eslint-disable-next-line no-undef
...
const getUserSubmissions = async (username) => {
  ...
  try {
    ...
    const heatmapOptionValues = await page.$$eval(`${heatmapSelectSelector} option`, (options) => options.map((option) => option.value));

    for (const optionValue of heatmapOptionValues) {
      await page.select(heatmapSelectSelector, optionValue);
      await page.waitForSelector('.heatmap-content', {
        visible: true,
      });
