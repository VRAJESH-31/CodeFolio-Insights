...
const getProblemsSolvedCountScore = (problemsCount) => {
  ...
  const codingScore =
    easyProblemsSolved * 2 + mediumProblemsSolved * 4 + hardProblemsSolved * 8;

  // 2. Ratio scores (Kept Linear/Ratio)
  // Avoid division by zero if totalProblemsSolved is 0
  const mediumProblemsRatioScore =
    totalProblemsSolved === 0
      ? 0
      : Math.min(
          100,
          (mediumProblemsSolved / (0.5 * totalProblemsSolved)) * 100,
        );
  const hardProblemsRatioScore =
    totalProblemsSolved === 0
      ? 0
  ...
  const score =
    scaledCodingScore * 0.7 +
    mediumProblemsRatioScore * 0.2 +
    hardProblemsRatioScore * 0.1;
...
const getContestPerformanceScore = (contestData) => {
  ...
  const contestTopPercentage =
    contestData.userContestRanking?.topPercentage || 100;
  ...
  const contestRatingScore = Math.max(
    0,
    100 * Math.pow(0.97, contestTopPercentage),
  );

  const score = Math.max(
    contestRatingScore,
    contestCountScore * 0.2 + contestRatingScore * 0.8,
  );
...
const getSubmissionConsistencyScore = (submissionCalendarData) => {
  const { currentStreak, maxStreak, activeDays } = getStreaksAndActiveDays(
...
const getBadgesScore = (badgesData) => {
  ...
  const contestBadge = badgesData['badges'].filter(
    (badge) => badge['category'] === 'COMPETITION',
  );
...
const getTopicWiseProblemsScore = (topicWiseProblemsData) => {
  ...
  const advancedProblemsCount = topicWiseProblemsData['advanced'].reduce(
    (problemsCount, topicProblemsSolved) =>
      problemsCount + topicProblemsSolved['problemsSolved'],
    0,
  );
  const intermediateProblemsCount = topicWiseProblemsData[
    'intermediate'
  ].reduce(
    (problemsCount, topicProblemsSolved) =>
      problemsCount + topicProblemsSolved['problemsSolved'],
  ...
  const fundamentalProblemsCount = topicWiseProblemsData['fundamental'].reduce(
    (problemsCount, topicProblemsSolved) =>
      problemsCount + topicProblemsSolved['problemsSolved'],
    0,
  );
  const totalProblemsCount =
    advancedProblemsCount +
    intermediateProblemsCount +
    fundamentalProblemsCount;

  // Avoid division by zero
  if (totalProblemsCount === 0) return 0;

  const advancedProblemsCountScore = Math.min(
    100,
    (advancedProblemsCount / totalProblemsCount) * 200,
  ); // Scaled up to give impact
  const intermediateProblemsCountScore = Math.min(
    100,
    (intermediateProblemsCount / totalProblemsCount) * 200,
  ); // Scaled up to give impact

  // New Scaling: The *2 was removed and the weights adjusted to make it harder to max out.
  const score =
    advancedProblemsCountScore * 0.6 + intermediateProblemsCountScore * 0.4;
