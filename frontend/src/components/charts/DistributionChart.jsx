...
const DistributionChart = ({ data, title = 'Problem Difficulty', className = '', includeLabels = true }) => {
  const total = data?.reduce((acc, item) => acc + item.value, 0) || 0;

  if (!total) return <EmptyState title={title} message="No Data Available" className={className} Icon={PieChartIcon} />;
            ...
            <PieChart>
              <Pie data={data} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={4} dataKey="value" stroke="none">
                {data.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={entry.color.gradient} />
                ))}
              </Pie>
              <Tooltip
                contentStyle={{
                ...
                <div key={item.name} className="flex items-center justify-between p-3 px-5 rounded-2xl bg-gray-50/50 border border-gray-100 group transition-all hover:bg-white hover:shadow-md">
                  <div className="flex items-center gap-3 min-w-0">
                    <div
                      ...
                      style={{
                        backgroundColor: item.color.gradient,
                      }}
                    />
                    <div className="text-[12px] font-black text-gray-500 uppercase tracking-widest truncate">{item.name}</div>
                  </div>
                  <div className="text-lg font-black text-gray-800 tabular-nums group-hover:scale-110 transition-transform ml-4" style={{ color: item.color.gradient }}>
                    {item.value}
                  </div>
                </div>
              ))}
