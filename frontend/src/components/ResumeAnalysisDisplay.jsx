      ...
      <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600 font-medium mt-1">
                Comprehensive Resume Assessment Report
              </p>
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/60 p-8 hover:shadow-2xl transition-all duration-500">
            ...
            <ScoreMeter
              score={overallScore}
              scoreComparison={resumeAnalysis.scoreComparison}
            />
          </div>
        ...
        <div className="pt-4 space-y-6">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 mt-2">
              Detailed analysis of each resume component
            </p>
          </div>
          {Object.keys(resumeAnalysis.scoreAnalysis).map((sectionKey) => {
            const section = resumeAnalysis.scoreAnalysis[sectionKey];

            if (sectionKey === 'section' || sectionKey === 'jobDescription')
              return null;

            return (
              <div
                key={sectionKey}
                className={`${getSectionGradient(section.score)} backdrop-blur-sm p-6 rounded-3xl shadow-xl border transition-all duration-300 hover:shadow-2xl hover:scale-[1.005]`}
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div
                    className={`px-4 py-2 rounded-2xl text-sm font-bold text-white bg-gradient-to-r ${getScoreColorClasses(section.score)} shadow-lg`}
                  >
                    {section.score}%
                  </div>
                </div>
                  ...
                  {section.analysis.map(
                    (line, lineIndex) =>
                      !!line.trim() && (
                        <li
                          key={lineIndex}
                          className="flex items-start gap-3 p-3 bg-white/50 rounded-xl backdrop-blur-sm"
                        >
        ...
        <div className="pt-4 space-y-6">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 mt-2">
              Detailed bullet point analysis and recommendations
            </p>
          </div>
          {Object.keys(resumeAnalysis.scoreAnalysis.section).map(
            (sectionKey) => {
              const section = resumeAnalysis.scoreAnalysis.section[sectionKey];
              const isPointAnalysisVisible =
                pointAnalysisVisibility[sectionKey];
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-left">
                            ...
                            <p className="text-sm text-gray-600">
                              Click to{' '}
                              {isPointAnalysisVisible ? 'collapse' : 'expand'}{' '}
                              detailed analysis
                            </p>
                          </div>
                        </div>
                      ...
                      {isPointAnalysisVisible && (
                        <div className="space-y-4 animate-fadeIn">
                          {section.pointAnalysis.map((pointData) => (
                            <div
                              key={uuid()}
                              className="p-5 bg-white/80 rounded-2xl backdrop-blur-sm border border-white/60 shadow-lg hover:shadow-xl transition-all duration-300"
                            >
                              <div className="flex justify-between items-center mb-3">
                                ...
                                <div
                                  className={`px-3 py-1 rounded-full text-xs font-bold ${pointData.score >= 8 ? 'bg-green-100 text-green-800 border border-green-200' : pointData.score >= 6 ? 'bg-amber-100 text-amber-800 border border-amber-200' : 'bg-rose-100 text-rose-800 border border-rose-200'}`}
                                >
        ...
        <div className="text-center py-8">
          <div className="bg-gradient-to-r from-blue-600 to-purple-700 rounded-3xl p-8 shadow-2xl">
            ...
            <p className="text-blue-100 mb-6 max-w-2xl mx-auto">
              Upload a new resume or implement the suggestions above to boost
              your score and land more interviews.
            </p>
            <button
              onClick={onUploadAgain}
