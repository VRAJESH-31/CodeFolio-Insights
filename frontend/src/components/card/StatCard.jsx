...
const StatCard = ({ title, value, change, Icon, color, index }) => {
  ...
  return (
    ...
    >
      <div
        className={`absolute top-0 right-0 w-24 h-24 rounded-full -mr-8 -mt-8 opacity-10 bg-current ${text}`}
      />
      ...
      <div className="flex justify-between items-start relative z-10">
        ...
        <div
          className={`p-4 rounded-2xl bg-gradient-to-br ${gradient} group-hover:scale-110 transition-transform shadow-lg`}
        >
      ...
      <div className="mt-4 w-full bg-gray-100 rounded-full h-1.5 overflow-hidden">
        <div
          className={`h-full rounded-full bg-gradient-to-r ${gradient.replace('100', '500').replace('200', '600')} animate-shimmer`}
          style={{ width: '85%' }}
        ></div>
      </div>
    </div>
