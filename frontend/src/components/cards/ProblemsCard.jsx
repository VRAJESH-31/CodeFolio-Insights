      ...
      <div className="flex items-center gap-6">
        <div className="flex-shrink-0">
          <ResponsiveContainer width={chartSize} height={chartSize}>
            <PieChart>
              <Pie data={problemsData} dataKey="value" innerRadius={chartSize * 0.3} outerRadius={chartSize * 0.45} paddingAngle={3} startAngle={90} endAngle={-270} stroke="none">
        ...
        <div className="flex-grow space-y-2">
          {problemsData.map((item) => (
            <div key={item.name} className="flex justify-between items-center bg-gray-50/50 px-3 py-1.5 rounded-xl border border-gray-100 hover:border-gray-200 transition-all group/item">
              <span className="font-bold text-xs uppercase tracking-wider" style={{ color: item.color }}>
                {item.name}
              </span>
