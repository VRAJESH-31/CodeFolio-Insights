...
const ApiProjectCard = ({ project, onDeleteRequest, onEditRequest }) => {
      ...
      <div className="relative space-y-5">
        ...
        <div className="flex justify-between items-start">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <h3 className="text-lg font-black text-slate-800 truncate max-w-[180px]">{project.name}</h3>
              {isDefault && (
          ...
          <div className="flex items-center gap-1">
            <button onClick={onEditRequest} className="p-2 text-slate-300 rounded-xl transition-all" title="Edit Project">
              <Edit2 className="w-6 h-6 text-amber-500" />
            </button>
            {!isDefault && (
              <button onClick={onDeleteRequest} className="p-2 text-slate-300 rounded-xl transition-all" title="Delete Project">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2 bg-slate-50 border border-slate-100 p-3 rounded-xl transition-all group-hover:border-slate-200">
            <code className="text-xs font-mono text-slate-600 flex-1 truncate font-medium">{showApiKey ? project.apiKey : '••••••••••••••••••••••••'}</code>
            <div className="flex items-center gap-1">
              <button onClick={() => setShowApiKey(!showApiKey)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-white rounded-lg transition-all" title={showApiKey ? 'Hide Key' : 'Show Key'}>
                {showApiKey ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
              </button>
              <button onClick={handleCopy} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-white rounded-lg transition-all" title="Copy Key">
        ...
        <div className="pt-2 flex flex-col gap-3">
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={() =>
                toast.success('Dashboard Coming Soon! ✨', {
                  icon: '🚀',
                })
              }
            ...
            <button onClick={handleSetDefault} disabled={isDefault || isUpdatingDefault} className={`flex justify-center items-center px-4 py-2.5 font-black rounded-xl text-[10px] uppercase tracking-widest transition-all border ${isDefault ? 'bg-green-50 text-green-600 border-green-100 cursor-default' : 'bg-white text-slate-600 border-slate-200 hover:border-blue-400 hover:text-blue-600 shadow-sm'}`}>
          ...
          <div className="flex justify-between items-center px-1">
            ...
            <span className="text-[10px] font-bold text-slate-400 tracking-tight">{formattedDate}</span>
          </div>
        </div>
      </div>
    </div>
