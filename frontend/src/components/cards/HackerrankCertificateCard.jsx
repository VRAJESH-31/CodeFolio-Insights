...
const HackerrankCertificateCard = ({ data, className = '', isExpanded = false }) => {
  const { certificate, certificate_image, completed_at, score, test_unique_id } = data.attributes;

  const formattedDate = completed_at.split('T')[0];
  ...
  return (
    <div className={`bg-white rounded-2xl border border-gray-100 shadow-md overflow-hidden flex flex-col h-full group relative ${className}`}>
      ...
      <div className={`relative aspect-[4/3] overflow-hidden bg-gray-50 shrink-0 border-b border-gray-100`}>
        <img src={certificate_image} alt={`${certificate?.label} Certificate`} className="w-full h-full object-cover" />
        ...
        <div className="absolute inset-0 bg-emerald-900/90 flex flex-col justify-center p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <p className={`text-white ${isExpanded ? 'text-sm' : 'text-[11px]'} font-medium leading-relaxed italic text-center`}>&quot;{certificate?.description}&quot;</p>
        </div>
      </div>
      ...
      <div className={`${isExpanded ? 'p-4' : 'p-2'} flex flex-col flex-1`}>
        ...
        <div className="flex justify-between items-start gap-1 mb-1">
          <div className="flex-1">
            <h3 className={`${isExpanded ? 'text-base' : 'text-[10px]'} font-bold text-gray-800 leading-[1.2] truncate-2-lines`}>{certificate?.level ? `${certificate?.label} (${certificate?.level})` : certificate?.label}</h3>
          </div>
        ...
        <div className={`flex items-center justify-between mt-auto ${isExpanded ? 'pt-3' : 'pt-1.5'} border-t border-gray-50`}>
          ...
          <a href={certificate_image || `https://www.hackerrank.com/certificates/${test_unique_id}`} target="_blank" rel="noopener noreferrer" className={`flex items-center gap-1 ${isExpanded ? 'text-sm' : 'text-[8px]'} font-bold text-blue-600 hover:text-blue-800 transition-colors`}>
            verify
