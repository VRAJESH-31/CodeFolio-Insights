...
const StatCard = ({ title, value, change, Icon, color, index }) => {
  ...
  return (
    <div className="bg-white/90 backdrop-blur-sm rounded-2xl border border-gray-100 shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 p-6 group relative overflow-hidden" style={{ animationDelay: `${index * 100}ms` }}>
      ...
      <div className="flex justify-between items-start relative z-10">
        ...
        <div className={`p-4 rounded-2xl bg-gradient-to-br ${gradient} group-hover:scale-110 transition-transform shadow-lg`}>{Icon && <Icon className={`w-5 h-5 ${text}`} />}</div>
      </div>
