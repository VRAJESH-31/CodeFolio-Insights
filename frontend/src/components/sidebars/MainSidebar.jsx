...
const MainSidebar = () => {
  ...
  return (
    ...
    >
      <div
        className={`relative flex items-center h-20 border-b border-gray-50 transition-all duration-500 ${isSidebarCollapsed ? 'justify-center pl-4' : 'justify-between pl-6'}`}
      >
      ...
      <nav
        className={`flex-1 ${isSidebarCollapsed ? 'overflow-visible' : 'overflow-y-auto'} py-6 px-4 custom-scrollbar`}
      >
              ...
              return (
                <li key={item.name} className="relative">
                  ...
                  >
                    ...
                    >
                      <div
                        className={`p-2 rounded-xl transition-all ${isActive ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-50 group-hover:bg-white'}`}
                      >
                        {item.isCustomIcon ? (
                          <img
                            src={item.iconPath}
      ...
      <div
        className={`p-4 border-t border-gray-50 ${isSidebarCollapsed ? 'flex justify-center' : ''}`}
      >
        <details className="group">
          <summary
            className={`flex items-center list-none cursor-pointer p-2 rounded-2xl hover:bg-gray-50 transition-all ${isSidebarCollapsed ? 'justify-center' : ''}`}
          >
            <div className="relative flex-shrink-0">
              <img
