...
const DeleteModal = ({ isOpen, onClose, onDelete, isPending, title, description, cancelText = 'Cancel', submitText = 'Delete', loadingText = 'Deleting...', icon: Icon }) => {
        ...
        <div className="relative space-y-6">
          ...
          <div className="flex justify-between items-start">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-red-50 rounded-2xl flex items-center justify-center border border-red-100 flex-shrink-0">{Icon && <Icon className="w-6 h-6 text-red-600" />}</div>
          ...
          <div className="space-y-4">
            <div className="text-slate-500 font-medium leading-relaxed">{description}</div>
          </div>
          ...
          <div className="flex gap-4 pt-2">
            ...
            <button onClick={onDelete} disabled={isPending} className="flex-[1.5] px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest bg-red-600 text-white shadow-lg shadow-red-500/20 hover:shadow-red-500/40 hover:bg-red-700 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:scale-100 flex items-center justify-center gap-2">
              {isPending ? (
                <>
