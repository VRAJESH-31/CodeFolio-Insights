...
const EditModal = ({ isOpen, onClose, onSubmit, initialValue, isPending, title, description, inputLabel, inputPlaceholder, submitText = 'Save', loadingText = 'Saving...', cancelText = 'Cancel', updatedProjectName, setUpdatedProjectName, icon: Icon }) => {
  useEffect(() => {
    if (initialValue !== undefined) {
      setUpdatedProjectName(initialValue);
    }
  }, [initialValue]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (updatedProjectName.trim() && updatedProjectName !== initialValue) {
      onSubmit(updatedProjectName.trim());
    } else if (updatedProjectName === initialValue) {
      onClose();
    }
  };
          ...
          <div className="flex justify-between items-start">
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-amber-50 rounded-2xl flex items-center justify-center border border-amber-100 flex-shrink-0">{Icon && <Icon className="w-6 h-6 text-amber-600" />}</div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-3">
              ...
              <div className="relative group">
                ...
                <input type="text" value={updatedProjectName} onChange={(e) => setUpdatedProjectName(e.target.value)} placeholder={inputPlaceholder} className="w-full pl-12 pr-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:bg-white transition-all font-bold text-slate-800 placeholder:font-medium placeholder:text-slate-400" autoFocus required />
              </div>
            </div>
            ...
            <div className="flex gap-4 pt-2">
              ...
              <button type="submit" disabled={isPending || !updatedProjectName?.trim() || updatedProjectName === initialValue} className="flex-[2] px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest bg-gradient-to-r from-amber-500 to-orange-600 text-white shadow-lg shadow-amber-500/20 hover:shadow-amber-500/40 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:scale-100">
                {isPending ? (
