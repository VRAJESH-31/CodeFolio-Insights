...
const AccountSettings = () => {
  ...
  const { mutate: updateApiKey, isPending: isUpdatingApiKey } =
    useUpdateApiKey();
              ...
              <div className="flex gap-2 relative group">
                ...
                <div className="flex gap-2">
                  <button
                    type="submit"
                    disabled={
                      isUpdatingApiKey || apiKeyForm.apiKey === user?.apiKey
                    }
                    className="flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-700 text-white font-black px-6 py-3.5 rounded-xl shadow hover:shadow-indigo-200 transition-all disabled:opacity-50 uppercase tracking-widest text-xs min-w-[100px]"
                  >
                    {isUpdatingApiKey ? (
      ...
      {user?.provider === 'credential' || user?.provider === 'both' ? (
        ...
        >
          ...
          <div className="space-y-4">
            ...
            {/* New Password */}
            <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 items-center">
              <label className="text-sm font-black text-slate-500 uppercase tracking-wider internal-label">
                New Password:
              </label>
              <div className="relative group">
                <input
        ...
        <div className="pt-6 border-t border-slate-100">
          <div className="flex items-center justify-between p-6 bg-slate-50 rounded-2xl border border-slate-100">
            <div className="flex items-start gap-4">
              <div
                className={`p-3 rounded-xl ${user?.enable2FA ? 'bg-green-100 text-green-600' : 'bg-slate-200 text-slate-500'}`}
              >
