...
const AppearanceSettings = () => {
  ...
  const themes = [
    ...
    {
      name: 'green',
      class: { bg: 'bg-green-500', border: 'border-green-500' },
    },
    {
      name: 'purple',
      class: { bg: 'bg-purple-500', border: 'border-purple-500' },
    },
    {
      name: 'orange',
      class: { bg: 'bg-orange-500', border: 'border-orange-500' },
    },
    ...
    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-blue-100/50 p-8">
                  ...
                  <div className="flex bg-white p-1 rounded-lg border border-gray-200 shadow-sm">
                    {['tab', 'sidebar'].map((type) => (
                      <button
                        ...
                        className={`px-3 py-1 text-xs font-bold rounded-md transition-all duration-200 capitalize ${
                          pageView[section.id] === type
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-around gap-4">
              {themes.map((theme) => (
                ...
                >
                  <div
                    className={`w-full h-full rounded-lg ${theme.class.bg}`}
                  ></div>
                  {themeVariant === theme.name && (
                    <div
                      className={`absolute -top-1 -right-1 w-4 h-4 ${theme.class.bg} rounded-full border-2 border-white`}
                    ></div>
                  )}
                </button>
              ))}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {layouts.map((l) => (
                <label
                  key={l}
                  className="flex items-center gap-3 p-3 rounded-xl border border-gray-200 hover:bg-blue-50/50 transition-all duration-300 cursor-pointer"
                >
                  <input
                    type="radio"
                    name="layout"
      ...
      <div className="mt-8 p-6 bg-gradient-to-r from-blue-50 to-purple-50 rounded-2xl border border-blue-200">
        ...
        <p className="text-sm text-gray-600 mt-2">
          Your preferences are synced and saved automatically to local storage.
        </p>
      </div>
    </div>
  );
