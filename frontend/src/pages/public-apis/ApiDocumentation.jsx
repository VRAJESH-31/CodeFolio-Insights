...
const ApiDocumentation = () => {
  ...
  const handleSelectEndpoint = (category, title) => {
    navigate(
      `/public-apis/documentation?category=${encodeURIComponent(category)}&title=${encodeURIComponent(title)}`,
    );
  };
                    ...
                    <tbody className="divide-y divide-slate-100 bg-white">
                      {currentEndpoint.parameters.map((param, idx) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <span
                              className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide ${param.status === 'required' ? 'bg-red-100 text-red-600' : 'bg-slate-100 text-slate-500'}`}
                            >
                              {param.status}
                            </span>
                          </td>
