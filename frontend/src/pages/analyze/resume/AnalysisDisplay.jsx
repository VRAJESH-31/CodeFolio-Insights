...
import { ScoreMeter, MemeContainer } from '@/components/export.js';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-cyan-50/30 py-8 px-4">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <UploadHeader icon={Sparkles} title="AI Resume Analysis" subtitle="Comprehensive Resume Assessment Report" buttonText="Upload New Resume" buttonIcon={UploadCloud} onAction={onUploadAgain} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <InsightCard title="Strengths" points={resumeAnalysis?.strengths} Icon={TrendingUp} PointIcon={CheckCircle} className="bg-gradient-to-br from-blue-50/80 via-white to-cyan-50/80 border-blue-200/60 backdrop-blur-sm" iconBg="bg-blue-100/80" iconColor="text-blue-600" pointIconColor="text-green-500" pointColor="text-blue-800" />

          <InsightCard title="Weaknesses" points={resumeAnalysis?.weaknesses} Icon={TrendingDown} PointIcon={AlertCircle} className="bg-gradient-to-br from-amber-50/80 via-white to-orange-50/80 border-amber-200/60 backdrop-blur-sm" iconBg="bg-amber-100/80" iconColor="text-amber-600" pointIconColor="text-amber-500" pointColor="text-amber-800" />

          <InsightCard title="Improvement Areas" points={resumeAnalysis?.improvements} Icon={Target} PointIcon={CheckCircle} className="bg-gradient-to-br from-green-50/80 via-white to-cyan-50/80 border-green-200/60 backdrop-blur-sm md:col-span-2" iconBg="bg-green-100/80" iconColor="text-green-600" pointIconColor="text-green-500" pointColor="text-green-800" />
        </div>

        {/* Resume Structure Deep Dive */}
        <div className="pt-4 space-y-6">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-black bg-gradient-to-r from-gray-800 to-purple-700 bg-clip-text text-transparent">Resume Structure Deep Dive</h2>
            ...
            return (
              <div key={sectionKey} className={`${getSectionGradient(section.score)} backdrop-blur-sm p-6 rounded-3xl shadow-xl border transition-all duration-300 hover:shadow-2xl hover:scale-[1.005]`}>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-xl font-bold capitalize text-gray-800">{sectionKey.replace(/([A-Z])/g, ' $1').trim()}</h3>
                  <div className={`px-4 py-2 rounded-2xl text-sm font-bold text-white bg-gradient-to-r ${getScoreColorClasses(section.score)} shadow-lg`}>{section.score}%</div>
                </div>
        ...
        <div className="pt-4 space-y-6">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-black bg-gradient-to-r from-gray-800 to-purple-700 bg-clip-text text-transparent">Section-wise Deep Dive</h2>
              ...
              <div key={sectionKey} className={`${getSectionGradient(section.score)} backdrop-blur-sm p-6 rounded-3xl shadow-xl border transition-all duration-300 hover:shadow-2xl hover:scale-[1.005]`}>
                ...
                {section.pointAnalysis && (
                  <div className="mt-6 pt-6 border-t border-white/50 space-y-4">
                    <button onClick={() => togglePointAnalysis(sectionKey)} className="w-full flex justify-between items-center p-4 bg-white/60 rounded-2xl backdrop-blur-sm hover:bg-white/80 transition-all duration-300 group">
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-purple-100 rounded-lg">
                          <FileText className="h-5 w-5 text-purple-600" />
                        </div>
                        <div className="text-left">
                          <h4 className="text-lg font-bold text-gray-800 group-hover:text-purple-700 transition-colors">Bullet Point Review</h4>
                          <p className="text-sm text-gray-600">Click to {isPointAnalysisVisible ? 'collapse' : 'expand'} detailed analysis</p>
                        </div>
                      </div>
                      <div className="p-2 bg-white rounded-full shadow-lg">{isPointAnalysisVisible ? <ChevronUp className="h-5 w-5 text-purple-600" /> : <ChevronDown className="h-5 w-5 text-purple-600" />}</div>
                    </button>
                      ...
                      <div className="space-y-4 animate-fadeIn">
                        {section.pointAnalysis.map((pointData) => (
                          <div key={uuid()} className="p-5 bg-white/80 rounded-2xl backdrop-blur-sm border border-white/60 shadow-lg hover:shadow-xl transition-all duration-300">
                            ...
                            <p className="text-gray-800 italic mb-4 p-3 bg-gray-50 rounded-lg border-l-4 border-blue-300">{pointData.point.original}</p>
        ...
        <div className="text-center py-8">
          <div className="bg-gradient-to-r from-blue-600 to-purple-700 rounded-3xl p-8 shadow-2xl">
            <h3 className="text-2xl font-black text-white mb-3">Ready to Improve Your Resume?</h3>
            <p className="text-blue-100 mb-6 max-w-2xl mx-auto">Upload a new resume or implement the suggestions above to boost your score and land more interviews.</p>
            <button onClick={onUploadAgain} className="inline-flex items-center gap-3 px-8 py-4 bg-white text-blue-700 font-bold rounded-2xl shadow-2xl hover:shadow-3xl transition-all duration-300 transform hover:scale-105">
